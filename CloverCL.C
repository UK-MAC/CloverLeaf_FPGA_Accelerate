...
void CloverCL::createBuffers(int x_max, int y_max, int num_states)
{
    ...
    xvel0_buffer_c = clCreateBuffer( context_c, CL_MEM_READ_WRITE, (x_max+6)*(y_max+5)*sizeof(double), NULL, &err);

    //yvel0_buffer_c = clCreateBuffer( context_c, CL_MEM_READ_WRITE, (x_max+5)*(y_max+5)*sizeof(double), NULL, &err);
    yvel0_buffer_c = clCreateBuffer( context_c, CL_MEM_READ_WRITE, (x_max+6)*(y_max+5)*sizeof(double), NULL, &err);

    //xvel1_buffer_c = clCreateBuffer( context_c, CL_MEM_READ_WRITE, (x_max+5)*(y_max+5)*sizeof(double), NULL, &err);
    xvel1_buffer_c = clCreateBuffer( context_c, CL_MEM_READ_WRITE, (x_max+6)*(y_max+5)*sizeof(double), NULL, &err);

    //yvel1_buffer_c = clCreateBuffer( context_c, CL_MEM_READ_WRITE, (x_max+5)*(y_max+5)*sizeof(double), NULL, &err);
    yvel1_buffer_c = clCreateBuffer( context_c, CL_MEM_READ_WRITE, (x_max+6)*(y_max+5)*sizeof(double), NULL, &err);

    //xarea_buffer_c = clCreateBuffer( context_c, CL_MEM_READ_WRITE, (x_max+5)*(y_max+4)*sizeof(double), NULL, &err);
    xarea_buffer_c = clCreateBuffer( context_c, CL_MEM_READ_WRITE, (x_max+6)*(y_max+4)*sizeof(double), NULL, &err);
...
void CloverCL::loadProgram(int xmin, int xmax, int ymin, int ymax)
{
    ...
    build_one_program(xmin, xmax, ymin, ymax, "accelerate_removeMemOp_double2_wg_singleWI_1921_3842_1922_1923_pipelineOpts_2Darray_rollingX_rollingY_finalIT_profile.aocx", &accelerate_prog);
